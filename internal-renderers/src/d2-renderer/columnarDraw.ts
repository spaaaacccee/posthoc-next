...
export type Ctx2D = Pick<
  ...
  "fillStyle" | "fillRect" | "beginPath" | "ellipse" | "fill"
>;
...
export function drawBody(
  ...
  ctx.fillStyle = resolveFill(store, store.fill[i]!, store.alpha[i]!, cache);
  const x = store.x[i]! * t.sx + t.x;
  const y = store.y[i]! * t.sy + t.y;
  if (store.kind[i] === CIRCLE) {
    const rx = store.size[i]! * t.sx;
    const ry = store.size[i]! * t.sy;
    ctx.beginPath();
    ctx.ellipse(ceil(x), ceil(y), ceil(rx), ceil(ry), 0, 0, 2 * PI);
    ctx.fill();
  } else {
    const w = store.size[i]! * t.sx;
    const h = store.size2[i]! * t.sy;
    ctx.fillRect(ceil(x), ceil(y), ceil(w) || 1, ceil(h) || 1);
  }
}
